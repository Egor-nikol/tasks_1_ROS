...
**Для студента:** преподаватель выдаёт этот репозиторий после публикации образа и добавления `.env` с digest. Без этого release-step `docker compose up` не сможет скачать ещё не опубликованный образ.
